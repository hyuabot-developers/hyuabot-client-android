...
class ShuttleTabJungangStationFragment @Inject constructor() : Fragment() {
    ...
    override fun onCreateView(
        ...
        val shuttleCampusAdapter = ShuttleRealtimeByDestinationListAdapter(
            ...
            emptyList(),
            onAlarmClick = { entry ->
                showAlarmDialogForStop("jungang_stn", R.string.shuttle_tab_jungang_station, entry.seq, entry.time, entry.stops.map { it.stop })
            }
        )
        ...
        val shuttleAdapter = ShuttleRealtimeByTimeListAdapter(
            ...
            emptyList(),
            onAlarmClick = { order ->
                showAlarmDialogForStop("jungang_stn", R.string.shuttle_tab_jungang_station, order.seq, order.time, order.stops.map { it.stop })
            }
        )
